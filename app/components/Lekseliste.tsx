...
const Lekseliste = () => {
   ...
   const completeLekse = (lekse: string) => {
      console.log("lekse", lekse);
      console.log("lekser", lekser);
      console.log("completedLekser", completedLekser);
      if (lekser.includes(lekse)) {
   ...
   const deleteLekse = (lekse: string) => {
      if (lekser.includes(lekse)) {
         setLekser(lekser.filter((item) => item != lekse));
      } else if (completedLekser.includes(lekse)) {
         setCompletedLekser(completedLekser.filter((item) => item != lekse));
   ...
   interface LekseProps {
      ...
      onSave: () => void;
      onDelete: (lekse: string) => void;
   }
   const Lekse = ({ lekse, index, onCompletetion, onEdit, onSave, onDelete }: LekseProps) => {
   ...
   const CompletedLekse = ({ lekse, index, onCompletetion, onDelete }: CompletedLekseProps) => {
      const inputRef = useRef<HTMLInputElement>(null);
